...
public boolean check(ArrayList<ChiTietCard> cartlist, String id2) {
		
		for(int i=0;i<cartlist.size();i++) {
			if(Integer.parseInt(id2)==cartlist.get(i).getIdProduct())
				return true;
		}
		return false;
	}
	public ArrayList<ChiTietCard> remove(ArrayList<ChiTietCard> cartlist, String id) {
		for(int i=0;i<cartlist.size();i++) {
			if(Integer.parseInt(id)==cartlist.get(i).getIdProduct()) {
